    ...
    if User.objects.filter(default_superuser=True).exists():
        ...
        if settings.ADMIN_PERSISTENT:
            ...
            if not username_unique_fail and not email_unique_fail:
                ...
                if not str(user.get_primary_email) == settings.ADMIN_MAIL:
                    print(f'{Fore.GREEN}Changing existing default superuser\'s email:')
                    print(f'From {user.get_primary_email} to {settings.ADMIN_MAIL}{Style.RESET_ALL}')
                    ...
                    old_email_address = user.get_primary_email

                    user.add_email_address(settings.ADMIN_MAIL, primary=True)

                    # Automatically verifiy a superuser's email address.
                    EmailAddress.objects.verify(
                        EmailAddress.objects.get(email_address=user.get_primary_email)
                    )
                ...
                print(f'E-Mail: {user.get_primary_email}{Style.RESET_ALL}\n')
            else:
